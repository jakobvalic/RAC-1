...
# =============================================================================
def zapomniSi(f):
    '''Dekorator memoizacije.'''
    rezultati = dict()
    def pametna(x):
        x = tuple(x) # Pretvorimo iz seznama, ker seznam ne more biti ključ v slovarju
...
def stevilo_mnozenj(dim):
    '''Izračuna najmanjše število množenj s pomočjo Belmann-Fordove enačbe.'''
    if len(dim) <= 1 :
        return 0 # Zaustavitveni pogoj
    a = dim[0][0] # Vrstice prve matrike
    b = dim[-1][1] # Stolpci zadnje matrike
    moznosti = []
    for k in range(1, len(dim)):
        prva = stevilo_mnozenj(dim[:k])
        druga = stevilo_mnozenj(dim[k:])
        moznosti.append(prva + druga + a * b * dim[k][0]) # Zmnožimo z vrsticami k-te matrike
    return min(moznosti)

def f(i, j):
    '''Ideja Belmann-Fordove enačbe.'''
    if i == j:
        return 0
    else:
        return min(
...
#     '((A1(A2(A3A4)))A5)'
# =============================================================================
def vrstni_red(dim, zac=1, kon=1):
    '''Vrne vrstni red množenja matrik, ki vrnejo najmanjše število množenj.'''
    if len(dim) <= 1:
        return (0, '')
    a = dim[0][0] # Vrstice prve matrike
    b = dim[-1][1] # Vrstice zadnje matrike
    moznosti = []
    for k in range(1, len(dim)):
        prva = stevilo_mnozenj(dim[:k], 1, zac + k)
        druga = stevilo_mnozenj(dim[k:], k, kon + k)
        mnozenj = prva[0] + druga[0] + a * b * dim[k][0]
        niz = '(A{}'.format(zac + prva[1])
        niz += druga[1] + 'A{})'.format(len(dim) + kon)
        moznosti.append((mnozenj, niz))
    return min(moznosti, key=lambda par: par[0]) # Element z najmanjšim številom množenj
