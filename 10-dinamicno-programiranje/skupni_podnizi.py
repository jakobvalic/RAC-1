...
def skupen_podniz(niz1, niz2):
    '''Poišče in vrne najdaljši podniz.'''
    if len(niz1) == 0 or len(niz2) == 0:
        return ''
    if niz1[0] == niz2[0]:
        return niz1[0] + skupen_podniz(niz1[1:], niz2[1:])
    # Ohranimo prvi niz in drugega nadaljujemo
    leviCel = skupen_podniz(niz1, niz2[1:])
    desniCel = skupen_podniz(niz1[1:], niz2)
    return max(desniCel, leviCel, key=len)
