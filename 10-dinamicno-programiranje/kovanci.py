...
def placilo_v2(znesek, kovanci):

    @lru_cache(maxsize=None)
    def pomozna(znesek):
        '''Vrne količino posameznih kovancev, da plačamo dani znesek s čim manj kovanci.'''
        if znesek == 0:
            return [0] * len(kovanci) # Ni nam treba več plačati

        moznosti = []
        for i, kovanec in enumerate(kovanci):
            if kovanec <= znesek:
                # Kako plačamo preostanek, minimum
                placilo = pomozna(znesek - kovanec).copy() # Nujno, drugače lru_cache hrani reference na isti seznam
                # Kako plačamo glavni del
                placilo[i] += 1
                moznosti.append(placilo)
        return min(moznosti, key=sum)

    return pomozna(znesek)



def placilo(znesek, kovanci):
    '''Optimizirana rešitev, s tem, da kovance uredimo in plačujemo z vedno večjimi.'''

    @lru_cache(maxsize=None)
    def pomozna(znesek, n=0):
        '''Vrne količino posameznih kovancev, da plačamo dani znesek s čim manj kovanci.'''
        if znesek == 0:
            return [0] * len(kovanci) # Ni nam treba več plačati

        moznosti = []
        for i, kovanec in enumerate(kovanci[n:]):
            if kovanec <= znesek:
                # Preostanek plačamo z istim ali večjim kovancem
                placilo = pomozna(znesek - kovanec, i + n).copy() # Nujno, drugače lru_cache hrani reference na isti seznam
                # Glavni del plačamo s kovancem, ki smo ga izbrali
                placilo[i + n] += 1
                moznosti.append(placilo)
        if len(moznosti) > 0:
            return min(moznosti, key=sum)
        else:
            return [float('inf')] * len(kovanci)
