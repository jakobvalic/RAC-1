...
# =============================================================================
def nahrbtnik(predmeti, velikost):
    '''Preprost nahrbtnik: Predmete lahko razdelimo na manjše kose.'''
    p = list(enumerate(predmeti))
    p.sort(key=lambda t: -t[1][1] / t[1][0]) # Urejamo padajoče po gostoti
    v = 0 # Koliko volumna smo že zasedli
    x = [0] * len(predmeti)
    for i in range(len(predmeti)):
        k = p[i][0] # Zaporedna številka iz osnovnega seznama
        vel = p[i][1][0]
        if v + vel <= velikost:
            x[k] = 1
            v += vel
            ...
            if v != velikost:
                x[k] = (velikost - v) / vel # Tisto, kar nam ostane volumna / velikostjo predmeta
            break
    return x
