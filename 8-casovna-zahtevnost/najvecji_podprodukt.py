...
def najvecji_podprodukt(sez):
    '''Vrne največji produkt strnjenega zaporedja števil v seznamu.'''
    najProd = None
    trenutni = 1
    for x in sez:
        if x == 0:
            ...
            trenutni *= x
        else: # x < 0. Vzamemo ga, če je produkt večji od 1. V drugem primeru začnemo znova.
            trenutni = max(1, trenutni * x)
        if najProd is None or trenutni > najProd:
            najProd = trenutni
    return najProd
...
def _validate_current_file():
    ...
    if Check.part():
        ...
        Check.current_part['token'] = 'eyJ1c2VyIjoxMTUsInBhcnQiOjExNjE2fQ:1eeU6Z:pcOSR9msGNXJ3zE1F302cMoTmIo'
