...
def koliko_ponikne(mafija):
    '''Izračuna, koliko denarja ponikne.'''
    if mafija.prazno:
        return 0
    return (koliko_ponikne(mafija.levo) + koliko_ponikne(mafija.desno)) - mafija.podatek[1]
# =====================================================================@010477=
# 2. podnaloga
...
# =============================================================================
def goljufi(mafija):
    '''Funkcija okrog funkcije, ki hrani imena goljufov.'''
    mnozica_goljufov = set()
    def koliko_ponikne(mafija):
        '''Izračuna, koliko denarja ponikne.'''
        if mafija.prazno:
            return 0
        utaja = 0
        if vsota_poddreves(mafija) > mafija.podatek[1]:  # Nekaj si je mafijec prihranil zase
            utaja += vsota_poddreves(mafija) - mafija.podatek[1]
            mnozica_goljufov.add((mafija.podatek[0], utaja))
        return utaja + koliko_ponikne(mafija.levo) + koliko_ponikne(mafija.desno)
    koliko_ponikne(mafija)
    return mnozica_goljufov

# TU SPODAJ JE NEKAJ NAROBE... REKURZIVNO NAROBE.

# def goljufi(mafija, goljuf=set()):
#     '''Vrne množico goljufov s količino denarja, ki so ga utajili.'''
#     if mafija.prazno:
#         return set()
#     utaja = 0
#     if vsota_poddreves(mafija) > mafija.podatek[1]: # Nekaj si je mafijec prihranil zase
#         utaja += vsota_poddreves(mafija) - mafija.podatek[1]
#         goljuf.add((mafija.podatek[0], utaja))
#     goljuf.union(goljufi(mafija.levo)).union(goljufi(mafija.desno))
#     return goljuf
#
#
# d1 = Drevo(('Mirsad', 1030), levo=Drevo(('Slobodan', 570), levo=Drevo(('Vid', 220), levo=Drevo(('Ervin', 140)), desno=Drevo(('Andraž', 200))), desno=Drevo(('Rok', 430), levo=Drevo(('Ivo', 190)), desno=Drevo(('Maks', 150)))), desno=Drevo(('Miro', 460), levo=Drevo(('Leopold', 210), levo=Drevo(('Lovro', 130)), desno=Drevo(('Tine', 190))), desno=Drevo(('Jože', 300), levo=Drevo(('Maj', 200)), desno=Drevo(('Andrej', 150)))))
# d2 = goljufi(Drevo(('Salvatore', 320), levo=Drevo(('Bernardo', 200), levo=Drevo(('Matteo', 50)), desno=Drevo(('Carlo', 100), levo=Drevo(('Rosalia', 70)), desno=Drevo(('Tommaso', 50)))), desno=Drevo(('Francesco', 120), levo=Drevo(('Giuseppe', 70)), desno=Drevo(('Antonio', 60)))))
# print(goljufi(d2))
...
# =============================================================================
def zasluzek(mafija):
    '''Vrne nabor (zasluzek majnhih rib, zasluzek velikih rib).'''
    majhne_ribe = 0
    velike_ribe = 0
    if mafija.prazno:
        return 0, 0
    if mafija.levo.prazno and mafija.desno.prazno:
        majhne_ribe += mafija.podatek[1]
    else:
        if mafija.podatek[1] > vsota_poddreves(mafija): # Gledamo brez utajitev
            velike_ribe += mafija.podatek[1] - vsota_poddreves(mafija)
    majhne_Lpoddrevesa, velike_Lpoddrevesa = zasluzek(mafija.levo)
    majhne_Dpoddrevesa, velike_Dpoddrevesa = zasluzek(mafija.desno)
    majhne_ribe += majhne_Lpoddrevesa + majhne_Dpoddrevesa
    velike_ribe += velike_Lpoddrevesa + velike_Dpoddrevesa
    return (majhne_ribe, velike_ribe)
